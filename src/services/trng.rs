...
use crate::{
    ...
    smccc::{FunctionId, OwningEntityNumber, SUCCESS, SmcReturn, SmcValue, smc_return_set},
};
...
impl From<TrngError> for SmcValue {
    fn from(e: TrngError) -> Self {
        SmcValue::from(e as i32)
    }
}
...
impl Trng {
    ...
    fn handle_smc_common(&self, regs: &mut SmcReturn) {
        ...
        if TrngPlatformImpl::TRNG_UUID.is_nil() {
            regs.set_single(TrngError::NotSupported.into());
            return;
        }

        match function.0 {
            ARM_TRNG_VERSION => regs.set_single(TRNG_VERSION.into()),
            ARM_TRNG_FEATURES => {
                let feature_id = in_regs[1] as u32;
                let result = if is_trng_fid(feature_id) {
                    SUCCESS.into()
                } else {
                    TrngError::NotSupported.into()
                };

                regs.set_single(result);
            }
            ARM_TRNG_GET_UUID => regs.set_uuid(TrngPlatformImpl::TRNG_UUID),
            ARM_TRNG_RND32 => self.trng_rnd32(regs),
            ARM_TRNG_RND64 => self.trng_rnd64(regs),
            _ => regs.set_single(TrngError::NotSupported.into()),
        }
    }
    ...
    fn trng_rnd32(&self, regs: &mut SmcReturn) {
        ...
        if nbits == 0 || nbits > TRNG_RND32_ENTROPY_MAXBITS {
            regs.set_single(TrngError::InvalidParams.into());
            return;
        }

        let mut ent = [0u64; 2];
        if let Err(e) = self.pool.lock().pack_entropy(nbits, &mut ent) {
            regs.set_single(e.into());
            return;
        }
    ...
    fn trng_rnd64(&self, regs: &mut SmcReturn) {
        ...
        smc_return_set!(regs, SUCCESS as u64, ent[2], ent[1], ent[0]);
    }
}
...
#[cfg(test)]
mod tests {
    use crate::smccc::smc_return_set;
    ...
    fn trng_version() {
        ...
        smc_return_set!(regs, ARM_TRNG_VERSION as u64);
        expected.set_single(TRNG_VERSION.into());
    ...
    fn trng_features() {
        ...
        // Supported feature
        smc_return_set!(regs, ARM_TRNG_FEATURES as u64, ARM_TRNG_RND32 as u64);
        expected.set_single(SUCCESS.into());
        trng.handle_smc_common(&mut regs);
        assert_eq!(regs, expected);

        // Unsupported feature
        smc_return_set!(regs, ARM_TRNG_FEATURES as u64, 0x8400_0000_u64);
        expected.set_single(TrngError::NotSupported.into());
        trng.handle_smc_common(&mut regs);
        assert_eq!(regs, expected);
    }
    ...
    fn trng_get_uuid() {
        ...
        smc_return_set!(regs, ARM_TRNG_GET_UUID as u64);
        trng.handle_smc_common(&mut regs);
    ...
    fn trng_rnd32_invalid_nbits() {
        ...
        // nbits == 0
        smc_return_set!(regs, ARM_TRNG_RND32 as u64, 0);
        expected.set_single(TrngError::InvalidParams.into());
        trng.handle_smc_common(&mut regs);
        assert_eq!(regs, expected);
    ...
    fn trng_rnd64_invalid_nbits() {
        ...
        // nbits = 0
        smc_return_set!(regs, ARM_TRNG_RND64 as u64, 0);
        expected.set_single(TrngError::InvalidParams.into());
        trng.handle_smc_common(&mut regs);
        assert_eq!(regs, expected);
    ...
    fn trng_rnd32_get_entropy() {
        ...
        smc_return_set!(regs, ARM_TRNG_RND32 as u64, nbits);
        let expected_entropy = (1u64 << nbits).wrapping_sub(1);
        smc_return_set!(expected, SUCCESS as u64, 0, 0, expected_entropy);
        trng.handle_smc_common(&mut regs);
        assert_eq!(regs, expected);
    ...
    fn trng_rnd64_get_entropy() {
        ...
        smc_return_set!(regs, ARM_TRNG_RND64 as u64, nbits);
        ...
        smc_return_set!(expected, SUCCESS as u64, u64::MAX, u64::MAX, u64::MAX);
        trng.handle_smc_common(&mut regs);
        assert_eq!(regs, expected);
    }
}
