...
pub fn read_cache_size(level: CacheLevel) -> CacheSize {
    ...
    let is_64bit_format = ((read_id_aa64mmfr2_el1() >> CCIDX_SHIFT) & CCIDX_MASK) != 0;
    let (num_sets, associativity) = if is_64bit_format {
        (
...
/// have the correct view of the data. This may invole flushing the caches and exiting cache
/// coherency.
...
pub fn flush_cache_levels(levels: RangeInclusive<u8>) {
...
pub fn flush_level1_cache() {
    flush_cache_levels(1..=1);
}

/// Flushes level 2 cache.
pub fn flush_level2_cache() {
