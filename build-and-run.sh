...
case "$PLAT" in
  qemu)
    ...
    if [[ "${GDB}" == 1 ]]; then
        GDB="gdb-multiarch target/$(TARGET)/$(BUILDTYPE)/rf-a-bl31 --eval-command='target remote :${GDB_PORT}'"
    fi
    make -C $TFA PLAT=qemu ${DEBUG} FVP_TRUSTED_SRAM_SIZE=512 CC=clang NEED_BL32=yes NEED_BL31=no bl1 bl2
    make PLAT=qemu ${DEBUG} all
    ln -fsr ${BL1} target
	ln -fsr ${BL2} target
	cd target && ${QEMU} ${QEMU_FLAGS} ${QEMU_WAIT}
    ;;
  fvp)
    if [[ "${RME:-}" == 1 ]]; then
        make -C $TFA PLAT=fvp FVP_TRUSTED_SRAM_SIZE=512 ENABLE_RME=1 NEED_BL31=no ${DEBUG} bl1 bl2
        make PLAT=fvp FEATURES=sel2,rme ${DEBUG} all
        make -C $TFA PLAT=fvp FVP_TRUSTED_SRAM_SIZE=512 ENABLE_RME=1 NEED_BL31=no ${DEBUG} \
