...
/// The AEM FVP requires cache maintenance operations on CPU/Cluster power down, beucase it does not
...
unsafe impl Cpu for AemGeneric {
    ...
    fn power_down_level0() {
        ...
        // The exclusive memory accesses are predicatable with AEM FVP's cache implementation.
        unsafe {
            disable_dcache();
        }

        // Safety: The exclusive memory accesses are predicatable with AEM FVP's cache
        // implementation.
        unsafe {
            flush_level1_cache();
