...
pub trait ManifestList {
    ...
    fn as_slice(&self) -> &[Self::Item] {
        // Safety: assuming that the `ManifestList` is valid, then the area pointed to is an array
        // of `self.size()` elements and lives for the same lifetime as the whole manifest.
    ...
    T: ManifestList,
{
	...
	// array of `self.size()` elements and lives for the same lifetime as the whole manifest.
        unsafe { &mut *slice_from_raw_parts_mut(self.ptr() as *mut T::Item, self.size() as usize) };
    ...
    T: ManifestList,
{
    fn drop(&mut self) {
        if self.checksum().is_some() {
            ...
            *self.checksum().unwrap() = new_checksum;
        }
    }
}
