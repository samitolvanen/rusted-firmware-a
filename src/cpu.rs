...
pub extern "C" fn cpu_reset_handler() {
    ...
    loop {
        ...
        if ops.end() {
            ...
            // Safety: `CpuOps::from_cpu` converted the `extern "C" fn()` type into a raw function
            // pointer. Now we are converting back to be able to call it.
...
pub extern "C" fn cpu_power_down(level: usize) {
    ...
    loop {
        ...
        if ops.end() {
            ...
            // Safety: `CpuOps::from_cpu` converted the `fn()` type into a raw function pointer. Now
            // we are converting back to be able to call it.
