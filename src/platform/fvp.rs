...
use crate::{
    aarch64::{dsb_ish, dsb_sy, isb, wfi},
    ...
    sysregs::{
        IccSre, MpidrEl1, ScrEl3, SctlrEl3, Spsr, read_mpidr_el1, read_scr_el3, read_sctlr_el3,
        write_cntfrq_el0, write_scr_el3,
    },
};
...
const WARM_ENTRYPOINT_FIELD: *mut u64 = ARM_SHARED_RAM_BASE as *mut u64;
...
const DEVICE2: MemoryRegion = MemoryRegion::new(DEVICE2_BASE, DEVICE2_BASE + DEVICE2_SIZE);
...
fn map_peripheral<T>(phyical_instance: PhysicalInstance<T>) -> UniqueMmioPointer<'static, T> {
    // Safety: Physical instances are unique pointers to peropherals. The addresses remains valid
    // after turning on the MMU because of the identity mapping of the `V2M_MAP_IOFPGA` region.
    unsafe { UniqueMmioPointer::new(NonNull::new(phyical_instance.pa() as *mut T).unwrap()) }
}

static FVP_PSCI_PLATFORM_IMPL: Mutex<Option<FvpPsciPlatformImpl>> = Mutex::new(None);
...
impl Platform for Fvp {
    ...
    fn init_before_mmu() {
        ...
        *FVP_PSCI_PLATFORM_IMPL.lock() = Some(psci_platform);
        ...
        unsafe {
            *WARM_ENTRYPOINT_FIELD = bl31_warm_entrypoint as *const () as u64;
        }
        dsb_sy();
    }
    ...
    fn map_extra_regions(idmap: &mut IdMap) {
        ...
        map_region(idmap, &DEVICE2, MT_DEVICE);
    }
...
static GIC_CONTEXT: Mutex<FvpGicContext> = Mutex::new(FvpGicContext::new());

pub struct FvpPsciPlatformImpl<'a> {
    power_controller: Mutex<FvpPowerController<'a>>,
    system: Mutex<FvpSystemPeripheral<'a>>,
    timer_control: Mutex<GenericTimerControl<'a>>,
    timer_ctl: Mutex<GenericTimerCtl<'a>>,
    cci550: Mutex<Cci5x0<'a>>,
}
...
impl FvpPsciPlatformImpl<'_> {
    ...
    pub fn new(
        ...
        Self {
            power_controller: Mutex::new(FvpPowerController::new(map_peripheral(power_controller))),
            system: Mutex::new(FvpSystemPeripheral::new(map_peripheral(system))),
            timer_control: Mutex::new(GenericTimerControl::new(map_peripheral(timer_control))),
            timer_ctl: Mutex::new(GenericTimerCtl::new(map_peripheral(timer_ctl))),
            cci550: Mutex::new(Cci5x0::new(map_peripheral(cci550))),
        }
    }
    ...
    fn power_domain_on_finish_common(&self, target_state: &PsciCompositePowerState) {
        ...
        if target_state.states[Self::CLUSTER_POWER_LEVEL] == FvpPowerState::Off {
            ...
            // TODO: check whether it is ok to enter coherency at this point.
            unsafe {
