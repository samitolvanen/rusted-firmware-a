...
impl TryFrom<TestHelperRequest> for Phase {
    ...
    fn try_from(value: TestHelperRequest) -> Result<Self, Self::Error> {
        match value {
            [0, ..] => Ok(Phase::SWdOverwriteSIMD),
            [1, ..] => Ok(Phase::SWdCheckSIMD),
            _ => Err(()),
        }
    }
}
...
fn test_simd_context_switch_helper(
    ...
    match Phase::try_from(ns_world_request)? {
        Phase::SWdOverwriteSIMD => {
            // SWd overwrites its SIMD vector registers with 0, 2, 4, .. (consecutive even numbers).
...
fn test_simd_context_switch(helper: &TestHelperProxy) -> Result<(), ()> {
    if current_el() == 2 {
        ...
        // EL2.
        warn!("Skipping SIMD context switch test.");
        return Ok(());
    }
    ...
    helper(helper_run_phase_request(Phase::SWdOverwriteSIMD))?;
    ...
    // Switch to the SWd side of the test.
    helper(helper_run_phase_request(Phase::SWdCheckSIMD))?;
