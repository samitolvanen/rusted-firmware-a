...
fn convert_access_rights(value: arm_ffa_spmc::MemoryAccessRights) -> Attributes {
    let mut access_rights = if value.contains(arm_ffa_spmc::MemoryAccessRights::DEVICE) {
        MT_DEVICE
    } else {
        MT_MEMORY_WB
    };

    if value.contains(arm_ffa_spmc::MemoryAccessRights::RW) {
        access_rights |= Attributes::PXN | Attributes::UXN;
    } else if value.contains(arm_ffa_spmc::MemoryAccessRights::R) {
        access_rights |= Attributes::PXN | Attributes::UXN | Attributes::READ_ONLY;
    }

    if value.contains(arm_ffa_spmc::MemoryAccessRights::NS) {
        access_rights |= Attributes::NS;
    }

    if value.contains(arm_ffa_spmc::MemoryAccessRights::USER) {
        access_rights |= Attributes::USER;
    }

    if !value.contains(arm_ffa_spmc::MemoryAccessRights::GLOBAL) {
...
impl KernelInterface for KernelSpace {
    ...
    fn map_memory(
        ...
        length: usize,
        perm: arm_ffa_spmc::MemoryAccessRights,
    ) -> Self::VirtualAddress {
        debug!(
...
impl SpInterface for Sp {
    ...
    fn map_memory(
        ...
        _length: usize,
        _perm: arm_ffa_spmc::MemoryAccessRights,
    ) -> Self::VirtualAddress {
        0
    }
    ...
    fn set_access_rights(
        &self,
        _va: Self::VirtualAddress,
        _perm: arm_ffa_spmc::MemoryAccessRights,
    ) {
    }
...
pub struct Spmc<'a> {
    pub libspmc: arm_ffa_spmc::spmc::Spmc<'a, KernelSpace, Spmd, Sp>,
}
...
impl<'a> Spmc<'a> {
    pub fn new() -> Self {
        ...
        let mut libspmc =
            arm_ffa_spmc::spmc::Spmc::new(&crate::services::SERVICES.spmd, kernel_space);
        ...
        let sp_pkg_lib1 = arm_ffa_spmc::manifest::SpPackage {
    ...
    pub fn handle_call_from_spmd(msg: &mut Interface) -> World {
        match SPMC.lock().libspmc.handle_msg_from_spmd(msg) {
            arm_ffa_spmc::NextComponent::Spmd => World::NonSecure,
            arm_ffa_spmc::NextComponent::Sp(_) => World::Secure,
        }
    }
