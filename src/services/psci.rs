...
pub trait PsciSpmInterface {
    ...
    /// Notify the SPM that the current core was turned on after CPU_SUSPEND.
...
impl Psci {
    ...
    /// point and whether the CPU was awaken from suspend.
    pub fn handle_cpu_boot(&self) -> (EntryPoint, bool) {
        let cpu_index = CoresImpl::core_index();
        let mut cpu = self.power_domain_tree.locked_cpu_node(cpu_index);
