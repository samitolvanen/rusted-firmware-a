...
impl ErrataManagement {
    ...
    fn handle_smc(regs: &mut SmcReturn, world: World) {
        ...
        match function.0 {
            ...
            EM_CPU_ERRATUM_FEATURES => regs.set_from(cpu_erratum_features(in_regs, world)),
...
impl Service for ErrataManagement {
    ...
    fn handle_secure_smc(&self, regs: &mut SmcReturn) -> World {
        Self::handle_smc(regs, World::Secure);
        World::Secure
    }

    #[cfg(feature = "rme")]
    fn handle_realm_smc(&self, regs: &mut SmcReturn) -> World {
        Self::handle_smc(regs, World::Realm);
        World::Realm
    }
}
...
fn cpu_erratum_features(regs: &[u64], world: World) -> Status {
...
mod tests {
    ...
    fn em_version_non_secure() {
        ...
        assert_eq!(regs.values(), [0x0001_0000]);
    }
