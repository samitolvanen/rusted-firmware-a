...
impl<'a> Gic<'a> {
    fn new(mut gic: GicV3<'a>) -> Self {
        debug!("GIC config: {:x?}", &PlatformImpl::GIC_CONFIG);
        
        Self::init_distributor(&mut gic, &PlatformImpl::GIC_CONFIG);
        debug!("GIC distributor initialized.");
        ...
        for redistributor_index in 0..PlatformImpl::CORE_COUNT {
            Self::init_redistributor(&mut gic, redistributor_index, &PlatformImpl::GIC_CONFIG);
            debug!("GIC distributor {} initialized.", redistributor_index);
        }

        // Calculate redistributor index for each CPU and store it for future use.
        let redistributor_indices = Self::calculate_redistributor_indices(&mut gic);
    ...
    /// Returns redistributor index that corresponds to the current core.
    fn current_redistributor_index(&self) -> usize {
        self.redistributor_indices[CoresImpl::core_index()]
    }

    /// Returns mapping: core index -> redistributor index
    ...
    fn init_distributor(gic: &mut GicV3, config: &GicConfig) {
        ...
        Self::configure_spis(gic, config);
    ...
    /// Initialize GIC redistributor with index `core_index`.
    fn init_redistributor(gic: &mut GicV3, core_index: usize, config: &GicConfig) {
        ...
        Self::configure_private_interrupts(gic, core_index, config);
    }
}

fn init_cpu_interface() -> Result<(), GICRError> {
    let gic_helper = GIC.get().unwrap();
    let mut gic = gic_helper.gic.lock();
    let redistributor_idx = gic_helper.current_redistributor_index();

    gic.redistributor_mark_core_awake(redistributor_idx)?;
...
pub fn init() {
    ...
    if let Err(e) = init_cpu_interface() {
