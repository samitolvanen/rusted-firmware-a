...
fn main() {
    ...
    println!(
        "cargo:rustc-link-arg-bin=bl32=-T{crate_dir}/{platform}_bl32.ld"
    );
    println!(
        "cargo:rustc-link-arg-bin=bl33=-T{crate_dir}/{platform}_bl33.ld"
    );
    ...
    if platform == "fvp" {
        println!(
            "cargo:rustc-link-arg-bin=realm=-T{crate_dir}/{platform}_realm.ld"
        );
        println!("cargo:rerun-if-changed={crate_dir}/{platform}_realm.ld");
    }
}
