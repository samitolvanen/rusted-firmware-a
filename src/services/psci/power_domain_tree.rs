...
impl NonCpuPowerNode {
    ...
    pub fn get_minimal_allowed_state_without_core(&self, cpu_index: usize) -> PlatformPowerState {
        let mut min_state = PlatformPowerState::OFF;
        let local_cpu_index = cpu_index - self.cpu_range.start;
        for (i, state) in self.requested_states.iter().enumerate() {
...
pub mod test_helpers {
    use super::*;
    /// Set the power state (both the local_state and the CPU requested states) of the CPU given by
