...
/// A function to call may be set for each secondary core. When that core starts it will call the
/// function and unset the entry.
...
/// Calls PSCI CPU_ON to start the secondary CPU with the given MPIDR.
pub fn start_secondary(mpidr: u64, entry: fn(u64) -> !, arg: u64) -> Result<(), psci::Error> {
    let core_index = PlatformImpl::core_position(mpidr);
    ...
    psci::cpu_on::<Smc>(mpidr, secondary_entry as _, arg)
}
