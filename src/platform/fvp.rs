...
unsafe impl Platform for Fvp {
    ...
    #[cfg(feature = "rme")]
    const RMM_NCOH_REGION_COUNT: usize = DEVICE_REGIONS.len();
    ...
    fn rme_prepare_manifest(manifest: &mut RmmBootManifest) {
        ...
        let mut ncoh = manifest.plat_ncoh_region.as_slice_mut();
        for (idx, region) in DEVICE_REGIONS.iter().enumerate() {
            ncoh[idx] = RmmMemoryBank {
                base: region.start().0 as u64,
                size: (region.end() - region.start()) as u64,
            }
        }
    }
