...
pub trait ManifestList {
    ...
    fn as_slice(&self) -> &[Self::Item] {
        ...
        // condition above.
        unsafe { &*slice_from_raw_parts(self.ptr() as *const Self::Item, self.size()) }
    }
    ...
    T: ManifestList,
{
    fn deref_mut(&mut self) -> &mut Self::Target {
        ...
        // the condition above.
        unsafe { &mut *slice_from_raw_parts_mut(self.list.ptr() as *mut T::Item, self.list.size()) }
    }
}
...
impl RmmBootManifest {
    ...
    ///
    /// The buffer provided must of size [`RMM_SHARED_BUFFER_SIZE`] and page-aligned.
    ///
    ...
    pub fn new<'a>(
        ...
        // Checks that the buffer has the correct size and alignment.
        let ptr = buf.as_ptr() as usize;
        assert!(ptr.is_multiple_of(0x1000));
        assert!(buf.len() == RMM_SHARED_BUFFER_SIZE);
        ...
        let mut ptr = ptr + size_of::<Self>();

        // Initializes all the lists in the manifest with the requested size, leaving the entries
        // uninitialized.
        create_list!(s.plat_dram, dram_size, ptr, checksum = true);
        create_list!(s.plat_console, console_size, ptr, checksum = true);
        create_list!(s.plat_ncoh_region, ncoh_region_size, ptr, checksum = true);
        create_list!(s.plat_coh_region, coh_region_size, ptr, checksum = true);
        create_list!(s.plat_smmu, smmu_size, ptr, checksum = true);

        // Recursively create the root complex list and the lists it references.
        {
            create_list!(
                s.plat_root_complex,
                root_complex_size.len(),
                ptr,
                checksum = false
            );
    ...
    ($name:ident, $elems:ty) => {
        ...
        impl ManifestList for $name {
            ...
            fn compute_sum_of_bytes(&self) -> usize {
                self.num_entries
                    + self.entries_ptr
                    + <[usize]>::ref_from_bytes(self.as_slice().as_bytes())
                        .unwrap()
                        .iter()
                        .copied()
                        .sum::<usize>()
...
impl ManifestList for RmmRootComplexList {
    ...
    fn compute_sum_of_bytes(&self) -> usize {
        self.num_entries
            + ((u32::from(self.rc_info_version) as usize) << 32 | self.padding as usize)
            + self.entries_ptr
            + self
                .as_slice()
                .iter()
...
impl ManifestList for RmmRootPortInfo {
    ...
    fn compute_sum_of_bytes(&self) -> usize {
        ((self.root_port_id as usize) << 48
            | (self.padding as usize) << 32
            | self.num_entries as usize)
            + self.entries_ptr
            + <[usize]>::ref_from_bytes(self.as_slice().as_bytes())
                .unwrap()
