...
// TRNG SMC function identifiers, as defined in the TRNG specification, Arm DEN 0098.
...
impl EntropyPool {
    ...
    fn pack_entropy(&mut self, nbits: usize, out: &mut [u64]) -> Result<(), TrngError> {
        ...
        // Mask off higher bits if only part of word requested
...
static POOL: SpinMutex<EntropyPool> = SpinMutex::new(EntropyPool::new());
...
fn handle_smc(regs: &[u64; 18]) -> SmcReturn {
    ...
    match function.0 {
        ...
        ARM_TRNG_GET_UUID => {
            let val = TrngPlatformImpl::TRNG_UUID.to_u128_le();
            [val as u64, (val >> 64) as u64].into()
        }
        ARM_TRNG_RND32 => trng_rnd32(x1 as usize),
        ARM_TRNG_RND64 => trng_rnd64(x1 as usize),
        _ => TrngError::NotImplemented.into(),
    }
}

/// Generate n bits of entropy for a SMC32 call
fn trng_rnd32(nbits: usize) -> SmcReturn {
    if nbits == 0 || nbits > TRNG_RND32_ENTROPY_MAXBITS {
        return TrngError::InvalidParams.into();
    }

    let mut ent = [0u64; 2];
    if let Err(e) = POOL.lock().pack_entropy(nbits, &mut ent) {
        return e.into();
    }

    match (nbits - 1) / 32 {
        0 => [SUCCESS as u64, 0, 0, ent[0]].into(),
        1 => [SUCCESS as u64, 0, (ent[0] >> 32), ent[0] & 0xFFFF_FFFF].into(),
        2 => [
            SUCCESS as u64,
            ent[1],
        ...
        .into(),
        _ => TrngError::InvalidParams.into(),
    }
}

/// Generate n bits of entropy for a SMC64 call
fn trng_rnd64(nbits: usize) -> SmcReturn {
    ...
    match (nbits - 1) / 64 {
        0 => [SUCCESS as u64, 0, 0, ent[0]].into(),
        1 => [SUCCESS as u64, 0, ent[1], ent[0]].into(),
        2 => [SUCCESS as u64, ent[2], ent[1], ent[0]].into(),
        _ => TrngError::InvalidParams.into(),
    }
}
...
mod tests {
    ...
    fn trng_get_uuid() {
        ...
        let ret = handle_smc(&regs);
        let actual_uuid =
            Uuid::from_u128_le(ret.values()[0] as u128 | ((ret.values()[1] as u128) << 64));
