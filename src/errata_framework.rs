    ...
    ($apply_from:expr, $fixed_in:expr) => {
        unsafe {
            naked_asm!(
                ...
                "b.lt 4f",    // If less, condition not met, return false
                "b.gt 3f",    // If greater, condition met, check against FIXED_IN
                "cmp x1, x3", // If equal, compare current.variant with apply_from.variant
                "b.lt 4f",    // If less, condition not met, return false

                "3:", // Check if current < FIXED_IN
                "cmp x2, x6", // Compare current.revision with fixed_in.revision
                "b.lt 2f",    // If less, condition met, return true
                "b.gt 4f",    // If greater, condition not met, return false
                "cmp x1, x5", // If equal, compare current.variant with fixed_in.variant
                "b.lt 2f",    // If less, condition met, return true
                "b.ge 4f",    // If greater or equal, condition not met, return false
...
pub extern "C" fn apply_reset_errata() {
    naked_asm!(
        ...
        "cmp  w11, {reset_type}",
        "b.ne 4f", // Skip if not Reset type

        // Call check()
        "ldr  x11, [x9, #{check_offset}]",
        "blr  x11",
        "cbz  x0, 4f", // Skip if check() returns false
