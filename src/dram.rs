...
/// lazy_indirect! {
///     /// Rustdoc comment for FOO.
///     pub FOO, u64, 42, unsafe(link_section = ".dram_zeroed")
/// }
/// ```
#[allow(unused_macros)]
macro_rules! lazy_indirect {
    ($(#[$attributes:meta])* $name:ident, $t:ty, $init:expr $(, $raw_attributes:meta)*) => {
        $(#[$attributes])*
        static $name: spin::Lazy<&$t> = spin::Lazy::new(|| {
            $(#[$raw_attributes])*
            static RAW: $crate::dram::SyncUnsafeCell<core::mem::MaybeUninit<$t>> =
                $crate::dram::const_zeroed();
            // SAFETY: This is the only place where we create a reference to the contents of this
            // `SyncUnsafeCell`, and it only happens once during the initialisation of the `Lazy`.
            unsafe { &mut *RAW.get() }.write($init)
        });
    };
    ($(#[$attributes:meta])* pub $name:ident, $t:ty, $init:expr $(, $raw_attributes:meta)*) => {
        $(#[$attributes])*
        pub static $name: spin::Lazy<&$t> = spin::Lazy::new(|| {
            $(#[$raw_attributes])*
            static RAW: $crate::dram::SyncUnsafeCell<core::mem::MaybeUninit<$t>> =
                $crate::dram::const_zeroed();
            // SAFETY: This is the only place where we create a reference to the contents of this
